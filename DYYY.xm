			...
			if (!cityName || cityName.length == 0) {
				...
				if (cachedData) {
					...
					dispatch_async(dispatch_get_main_queue(), ^{
					  ...
					  if ([currentLabelText containsString:@"IP位置："]) {
						  NSRange range = [currentLabelText rangeOfString:@"IP位置："];
						  ...
						  if (currentLabelText.length > 0 && ![displayLocation isEqualToString:@"未知"]) {
							  label.text = [NSString stringWithFormat:@"%@  IP位置：%@", currentLabelText, displayLocation];
						  } else if (![displayLocation isEqualToString:@"未知"]) {
							  label.text = [NSString stringWithFormat:@"IP位置：%@", displayLocation];
						  }
					  }
