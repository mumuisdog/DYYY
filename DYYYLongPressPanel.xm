    ...
    if (enableApiDownload && apiKey.length > 0) {
        ...
        apiDownload.action = ^{
            ...
            if (shareLink.length == 0) {
                [DYYYManager showToast:@"無法獲取分享連結"];
                return;
            }
            // 使用封装的方法进行解析下载
...
if (enableCopyText) {
    ...
    copyText.action = ^{
        ...
        [DYYYManager showToast:@"文案已複製到剪貼簿"];
        AWELongPressPanelManager *panelManager = [%c(AWELongPressPanelManager) shareInstance];
        [panelManager dismissWithAnimation:YES completion:nil];
    };
...
if (enableCopyLink) {
    ...
    copyShareLink.describeString = @"複製連結";
        ...
        [DYYYManager showToast:@"分享連結已複製到剪貼簿"];
        AWELongPressPanelManager *panelManager = [%c(AWELongPressPanelManager) shareInstance];
        [panelManager dismissWithAnimation:YES completion:nil];
    };
...
if (enableFilterUser) {
    ...
    filterKeywords.action = ^{
        ...
        [DYYYBottomAlertView showAlertWithTitle:@"過濾使用者影片"
                               ...
                               cancelButtonText:@"管理過濾清單"
                              confirmButtonText:actionButtonText
                                   cancelAction:^{
            DYYYKeywordListView *keywordListView = [[DYYYKeywordListView alloc] initWithTitle:@"過濾使用者清單" keywords:userArray];
                ...
                [DYYYManager showToast:@"過濾使用者清單已更新"];
            };
        ...
        confirmAction:^{
            ...
            if (userExists) {
                ...
                [DYYYManager showToast:@"已從過濾清單中移除此使用者"];
            } else {
                // 添加用户
                [updatedUsers addObject:currentUserFilter];
                [DYYYManager showToast:@"已新增此使用者至過濾清單"];
            }
            // 保存更新后的列表
